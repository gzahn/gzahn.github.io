...
Now there's one more file you need to copy in this new GitHub repository you just cloned. Navigate into this new "Data_Course/" directory with cd and then enter the following line of code:
...
Copy your token and paste it into a text document. To set it as an "environment variable" enter the following in your command-line terminal, replacing TOKEN with the actual token you received online (keep all quotes as-is):

        echo 'export GITHUB="TOKEN"' >> ~/.bashrc

You can now type the following to see/copy your command-line GitHub token:

        echo $GITHUB

Now, we're going to set things up so that hopefully, it will automatically enter this long token for you when you need to push things to GitHub from your command-line.

Enter the following (unchanged) into your terminal:

        echo "export GIT_ASKPASS=~/.git_password.sh"
